...
const App: React.FC = () => {
  ...
  // Busca dados do Python a cada 2 segundos
  const fetchScripts = async () => {
    try {
      const response = await fetch('/api/scripts');
      if (!response.ok) throw new Error('Erro ao conectar com o backend Python');
      const data = await response.json();
      setScripts(data);
      setError(null);
    } catch (err) {
      setError('Servidor Python Offline. Verifique se o servidor.py está rodando.');
      console.error(err);
    }
  };
  ...
  const toggleScript = async (path: string, currentStatus: ScriptStatus) => {
    ...
    try {
      ...
      if (response.ok) {
        fetchScripts(); // Atualiza imediato
      }
    } catch (err) {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#09090b] text-zinc-100 selection:bg-indigo-500/30">
      ...
      <main className="flex-1 flex flex-col min-w-0 relative">
        {error && (
          <div className="bg-rose-500/20 text-rose-400 px-4 py-2 text-xs font-bold text-center border-b border-rose-500/30">
            {error}
          </div>
        )}
        ...
        <header className="h-20 border-b border-zinc-800/50 bg-[#09090b]/80 backdrop-blur-md sticky top-0 z-30 flex items-center justify-between px-8">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm font-medium text-zinc-500">
              {filteredScripts.length} automações
            </span>
          </div>
          ...
          <div className="flex items-center gap-4">
            <div className="relative group">
              ...
              <input 
                ...
                className="bg-zinc-900/50 border border-zinc-800 rounded-lg py-2 pl-10 pr-4 w-72 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all"
              />
            </div>
            
            <button 
              onClick={handleRefresh}
              className={`p-2 rounded-lg border border-zinc-800 hover:bg-zinc-800/50 transition-all ${isRefreshing ? 'animate-spin text-indigo-400 border-indigo-500/30' : 'text-zinc-400'}`}
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6 pb-20">
            {filteredScripts.map((script) => (
              <ScriptCard 
                key={script.id} 
                script={script} 
                onAction={() => toggleScript(script.path, script.status)}
              />
            ))}
            ...
            {filteredScripts.length === 0 && (
              <div className="col-span-full py-20 flex flex-col items-center justify-center text-zinc-500 border-2 border-dashed border-zinc-800 rounded-2xl">
                ...
                <p className="text-lg font-medium">Nenhum script ativo nesta área</p>
                <button 
                  onClick={() => {setActiveArea('TODOS'); setSearchQuery('');}}
